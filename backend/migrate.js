...
switch (command) {
  case 'create':
    execSync(`npx wrangler d1 migrations create ${options}`, { stdio: 'inherit' });
    break;
  case 'diff':
    ...
    break;
  case 'apply:local':
    execSync(`npx wrangler d1 migrations apply ${options} --local`, { stdio: 'inherit' });
    break;
  case 'apply:remote':
    execSync(`npx wrangler d1 migrations apply ${options} --remote`, { stdio: 'inherit' });
    break;
  default:
